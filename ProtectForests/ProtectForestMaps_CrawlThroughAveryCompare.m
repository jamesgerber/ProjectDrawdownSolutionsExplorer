...
MapsAndDataFilename='ProtectForestMapsAndData2';
MapsAndDataFilename='';  % if you keep this empty, DataToDrawdownFigures
%won't do anything

BaseNSS=getDrawdownNSS; % this will make maps
BaseNSS.plotflag='on'; %if you set this to off, DataToDrawdownFigures
%won't make maps
...
lowadoptionraster=(isintact | wdpa5min==1) & treecover5min>.00 ;
...
%% some calculations for comparison with published solution
[~,~,lon2,lat2]=inferlonglat;
jj = isfinite(Effectiveness);
length(find(~isfinite(lowadoptionraster)))
length(find(~isfinite(currentadoptionraster)))
length(find(~isfinite(highadoptionraster)))

% (23.4°N–23.4°S), subtropical (23.4°–35° latitude), temperate (35°–50° latitude), and boreal (>50° latitude), 


ii=abs(lat2) < 23.4 & jj;
disp(' ')
disp tropical
disp(['current area = ' int2str(sum(currentadoptionraster(ii).*fma(ii))/1e6) ' Mha' ]);
disp(['current impact = ' int2str(sum(Effectiveness(ii).*currentadoptionraster(ii).*fma(ii))/1e6) ' Mt CO2=eq/year' ]);
disp(['avg Effectiveness = ' num2str(sum(Effectiveness(ii).*currentadoptionraster(ii).*fma(ii))/sum(currentadoptionraster(ii).*fma(ii))) ' t CO2 eq/ha' ]);
disp(['low amb area = ' int2str(sum(lowadoptionraster(ii).*fma(ii))/1e6) ' Mha' ]);
disp(['low amb impact = ' int2str(sum(Effectiveness(ii).*lowadoptionraster(ii).*fma(ii))/1e6) ' Mt CO2=eq/year' ]);
disp(['avg Effectiveness = ' num2str(sum(Effectiveness(ii).*lowadoptionraster(ii).*fma(ii))/sum(lowadoptionraster(ii).*fma(ii))) ' t CO2 eq/ha' ]);
disp(['high amb area = ' int2str(sum(highadoptionraster(ii).*fma(ii))/1e6) ' Mha' ]);
disp(['high amb impact = ' int2str(sum(Effectiveness(ii).*highadoptionraster(ii).*fma(ii))/1e6) ' Mt CO2=eq/year' ]);
disp(['avg Effectiveness = ' num2str(sum(Effectiveness(ii).*highadoptionraster(ii).*fma(ii))/sum(highadoptionraster(ii).*fma(ii))) ' t CO2 eq/ha' ]);

ii=abs(lat2) > 23.4 & abs(lat2) <=35 & jj;
disp(' ')
disp subtropical
disp(['current area = ' int2str(sum(currentadoptionraster(ii).*fma(ii))/1e6) ' Mha' ]);
disp(['current impact = ' int2str(sum(Effectiveness(ii).*currentadoptionraster(ii).*fma(ii))/1e6) ' Mt CO2=eq/year' ]);
